...
<div class="box">
            ...
            <div class="col-xs-12 col-sm-8" style="padding-left: 0;margin-bottom: 10px;">
                <div class="input-group">
                    <input type="text" name="kw" class="form-control search-query"
                           placeholder="<?= yii::t('user', 'search placeholder') ?>">
    ...
    <div class="box-body table-responsive no-padding clearfix">
                    ...
                    <td>
                        <?php if ($row['role'] == User::ROLE_ADMIN) { ?>
                            <i class="icon icon-user-md green" data-placement="top" data-rel="tooltip"
                               data-title="<?= yii::t('w', 'user_role_' . User::ROLE_ADMIN) ?>"></i>
                        <?php } else { ?>
                            <i class="icon icon-user" data-placement="top" data-rel="tooltip"
                               data-title="<?= yii::t('w', 'user_role_' . User::ROLE_DEV) ?>"></i>
                        <?php } ?>

                        <?php if ($row['status'] == User::STATUS_INVALID) { ?>
                            <i class="icon icon-ban-circle red" data-placement="top" data-rel="tooltip"
                               data-title="<?= yii::t('user', 'status to opposite 10') ?>"></i>
                        <?php } ?>
                        <?php if ($row['is_email_verified'] == User::MAIL_INACTIVE) { ?>
                            <i class="icon icon-envelope red" data-placement="top" data-rel="tooltip"
                               data-title="<?= yii::t('user', 'inactive') ?>"></i>
                        <?php } ?>
                    </td>
                    <td>
                        <div class="nav">
                            <li>
                                ...
                                >
                                    <li><a href="###" data-toggle="modal" data-target="#update-real-name"><i
                                                class="icon-pencil"></i> <?= yii::t('w', 'edit') ?></a></li>
                                    <li><a href="###" class="cnt-user-option" data-url-key="status-url"
                                           data-confirm="<?= yii::t('user', 'label status to opposite ' . $row['status']) ?>"><i
                                                class="<?= $row['status'] == User::STATUS_INVALID ? 'icon-ok-circle' : 'icon-ban-circle' ?>"></i> <?= yii::t('user', 'status to opposite ' . $row['status']) ?>
                                        </a></li>
                                    <li><a href="###" class="cnt-user-option" data-url-key="delete-url"
                                           data-confirm="<?= yii::t('user', 'js delete user') ?>"><i
                                                class="icon-trash"></i> <?= yii::t('w', 'delete') ?></a></li>
                                    <li class="divider"></li>
                                    <li><a href="###" class="cnt-user-option" data-url-key="role-url"
                                           data-confirm="<?= yii::t('user', 'label role to opposite ' . $row['role']) ?>"><i
                                                class="i"></i> <?= yii::t('user', 'role to opposite ' . $row['role']) ?>
                                        </a></li>
                                </ul>
                            </li>
        ...
        <!-- 模态框（Modal） -->
        <div class="modal fade" id="update-real-name" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
             aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="real-name" class="control-label"><?= yii::t('user', 'label real name') ?>
                                :</label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default"
                                data-dismiss="modal"><?= yii::t('user', 'btn cancel') ?></button>
